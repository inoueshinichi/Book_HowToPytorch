...
from p2ch12.mod_dataset_luna import Ct, LunaDataset
from p2ch13.model_unet import SegmentationMask, MaskTuple

clim=(-1000.0, 300)

def findPositiveSamples(datasetdir="", start_ndx=0, limit=10):
    ds = LunaDataset(datasetdir=datasetdir, sortby_str='label_and_size')
...
def showCandidate(series_uid, datasetdir="", batch_ndx=None, **kwargs):
    ds = LunaDataset(datasetdir, series_uid=series_uid, **kwargs)
    ...
    ct = Ct(series_uid, datasetdir)
    ct_t, pos_t, series_uid, center_irc = ds[batch_ndx]
    center_irc_index, center_irc_row, center_irc_col = center_irc
    ct_a = ct_t[0].numpy()
    ...
    subplot.set_title('index {}'.format(int(center_irc_index)), fontsize=30)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(20)
    plt.imshow(ct.hu_a[int(center_irc_index)], clim=clim, cmap='gray')

    subplot = fig.add_subplot(len(group_list) + 2, 3, 2)
    subplot.set_title('row {}'.format(int(center_irc_row)), fontsize=30)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(20)
    plt.imshow(ct.hu_a[:,int(center_irc_row)], clim=clim, cmap='gray')
    plt.gca().invert_yaxis()

    subplot = fig.add_subplot(len(group_list) + 2, 3, 3)
    subplot.set_title('col {}'.format(int(center_irc_col)), fontsize=30)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(20)
    plt.imshow(ct.hu_a[:,:,int(center_irc_col)], clim=clim, cmap='gray')
    plt.gca().invert_yaxis()
